...
int audio_record(int16_t *pcm_buf, int max_samples, volatile bool *stop_flag)
{
	...
	/* Full I2S reset sequence — critical for preventing DMA crashes
	 * after play→record transitions on ESP32-S3 */
	i2s_trigger(i2s_dev, I2S_DIR_TX, I2S_TRIGGER_PREPARE);  /* recover from ERROR if needed */
	i2s_trigger(i2s_dev, I2S_DIR_RX, I2S_TRIGGER_PREPARE);
	i2s_trigger(i2s_dev, I2S_DIR_RX, I2S_TRIGGER_DROP);
	i2s_trigger(i2s_dev, I2S_DIR_TX, I2S_TRIGGER_DROP);
	k_msleep(100);

	/* Reinitialize mem slabs — repairs any DMA-related free list corruption */
	...
	/* Re-configure both directions (resets DMA descriptors) */
	struct i2s_config tx_cfg = {
		.word_size = SAMPLE_BITS,
		.channels = NUM_CHANNELS,
		.format = I2S_FMT_DATA_FORMAT_I2S,
	...
	ret = i2s_configure(i2s_dev, I2S_DIR_RX, &rx_cfg);
	if (ret && ret != -EBUSY) {
		LOG_ERR("I2S RX reconfig failed: %d", ret);
	}
