...
#define OV3660_PRE_ISP_TEST     0x503d
...
static const struct ov3660_reg ov3660_default_regs[] = {
	...
	{0x5480, 0x01},
	{0x5000, 0xa7},
	/* Lens Correction Coefficients (from ESP-IDF reference) */
	{0x5800, 0x0C}, {0x5801, 0x09}, {0x5802, 0x0C},
	{0x5803, 0x0C}, {0x5804, 0x0D}, {0x5805, 0x17},
	{0x5806, 0x06}, {0x5807, 0x05}, {0x5808, 0x04},
	{0x5809, 0x06}, {0x580a, 0x09}, {0x580b, 0x0E},
	{0x580c, 0x05}, {0x580d, 0x01}, {0x580e, 0x01},
	{0x580f, 0x01}, {0x5810, 0x05}, {0x5811, 0x0D},
	{0x5812, 0x05}, {0x5813, 0x01}, {0x5814, 0x01},
	{0x5815, 0x01}, {0x5816, 0x05}, {0x5817, 0x0D},
	{0x5818, 0x08}, {0x5819, 0x06}, {0x581a, 0x05},
	{0x581b, 0x07}, {0x581c, 0x0B}, {0x581d, 0x0D},
	{0x581e, 0x12}, {0x581f, 0x0D}, {0x5820, 0x0E},
	{0x5821, 0x10}, {0x5822, 0x10}, {0x5823, 0x1E},
	{0x5824, 0x53}, {0x5825, 0x15}, {0x5826, 0x05},
	{0x5827, 0x14}, {0x5828, 0x54}, {0x5829, 0x25},
	{0x582a, 0x33}, {0x582b, 0x33}, {0x582c, 0x34},
	{0x582d, 0x16}, {0x582e, 0x24}, {0x582f, 0x41},
	{0x5830, 0x50}, {0x5831, 0x42}, {0x5832, 0x15},
	{0x5833, 0x25}, {0x5834, 0x34}, {0x5835, 0x33},
	{0x5836, 0x24}, {0x5837, 0x26}, {0x5838, 0x54},
	{0x5839, 0x25}, {0x583a, 0x15}, {0x583b, 0x25},
...
static void ov3660_log_color_regs(const struct device *dev)
{
	const struct ov3660_config *cfg = dev->config;
	static const uint16_t regs[] = {0x501f, 0x4300, 0x5000, 0x5001, 0x5003, 0x5580, 0x5583,
					 0x5584, 0x5588};

	for (size_t index = 0; index < ARRAY_SIZE(regs); index++) {
		uint8_t value = 0;

		if (ov3660_read_reg(&cfg->i2c, regs[index], &value) == 0) {
			LOG_INF("reg 0x%04x = 0x%02x", regs[index], value);
		}
	}
}
...
static int ov3660_configure_qvga_rgb565(const struct device *dev)
{
	...
	ret |= ov3660_write_reg(&cfg->i2c, 0x5003, 0x08);
	ret |= ov3660_write_reg(&cfg->i2c, OV3660_ISP_CONTROL_01, 0xa3);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5580, 0x06);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5583, 0x40);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5584, 0x2c);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5588, 0x00);

	/* Sharpness: manual mode, level 2 */
	ret |= ov3660_write_reg(&cfg->i2c, 0x5308, 0x00);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5300, 0x10);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5301, 0x10);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5302, 0x29);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5303, 0x28);
	ret |= ov3660_write_reg(&cfg->i2c, 0x5309, 0x10);
	ret |= ov3660_write_reg(&cfg->i2c, 0x530a, 0x10);
	ret |= ov3660_write_reg(&cfg->i2c, 0x530b, 0x04);
	ret |= ov3660_write_reg(&cfg->i2c, 0x530c, 0x06);

	/* AE target level 0 (balanced exposure) */
	ret |= ov3660_write_reg(&cfg->i2c, 0x3a0f, 0x30);
	ret |= ov3660_write_reg(&cfg->i2c, 0x3a10, 0x28);
	ret |= ov3660_write_reg(&cfg->i2c, 0x3a1b, 0x30);
	ret |= ov3660_write_reg(&cfg->i2c, 0x3a1e, 0x28);
	ret |= ov3660_write_reg(&cfg->i2c, 0x3a11, 0x60);
	ret |= ov3660_write_reg(&cfg->i2c, 0x3a1f, 0x14);

	/* Contrast: level 1 (slightly enhanced) */
	ret |= ov3660_write_reg(&cfg->i2c, 0x5586, 0x28);
#if defined(CONFIG_VIDEO_OV3660_COLOR_BAR_TEST)
	ret |= ov3660_write_reg(&cfg->i2c, OV3660_PRE_ISP_TEST, 0x80);
#else
	ret |= ov3660_write_reg(&cfg->i2c, OV3660_PRE_ISP_TEST, 0x00);
#endif
...
static int ov3660_ensure_initialized(const struct device *dev)
{
	...
	/* Software reset with delay, like ESP-IDF does */
